...
/**
 * Resolves the [TileBehavior] of each block from the decomp tileset metatile attributes, so the
 ...
 * the two games' differing numeric values do not matter. Ported from the OpenMMO codegen.
 */
class MetatileBehaviors(
  ...
  /** One behavior ordinal byte per tile, same order as the block data. */
  fun behaviorData(primaryTileset: String?, secondaryTileset: String?, blockBytes: ByteArray): String {
  ...
  companion object {
    fun read(rootDir: File): MetatileBehaviors {
      ...
      val idToCategory = behaviorIds.mapNotNull { (name, id) -> classifyBehavior(name)?.let { id to it } }.toMap()
      val (attributePaths, width) = parseAttributeIncbins(File(rootDir, "src/data/tilesets/metatiles.h"))
      ...
      return MetatileBehaviors(
          ...
          tilesetAttributeSymbol = parseTilesetAttributes(File(rootDir, "src/data/tilesets/headers.h")),
      )
    }

    fun readPrimaryCount(rootDir: File): Int {
      if (!File(rootDir, "include/fieldmap.h").exists()) return 512
      val re = Regex("""#define\s+NUM_METATILES_IN_PRIMARY\s+(\d+)""")
      return File(rootDir, "include/fieldmap.h").readLines().firstNotNullOfOrNull {
